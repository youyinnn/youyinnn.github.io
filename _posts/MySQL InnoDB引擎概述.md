...
- InnoDB 1.1开始，回收Undo页的工作独立为Purge Thread；
- InnoDB 1.2开始，刷脏页的工作独立为Page Cleaner Thread;
...
- **在一般情况下**，主键是行的唯一标识符，所以插入顺序一般是按照主键递增的顺序执行，不需要磁盘随机读取，若主键支持自动增长，那么这类情况的插入效率非常快；对于这样的**顺序自增的主键**，对应**聚集索引**；
